#!/bin/bash
# Step 0: tmux new -s sort_mix
# Step 1: salloc --nodes=2 --ntasks-per-node=8 --gres=gpu:8 --cpus-per-task=12 --mem=1175G --exclude master 
# 记得要检查分配到的节点有cuda-compat，并且安装了 ffmpeg，否则会报错
# Step 2: srun bash /x2robot_v2/xinyuanfang/projects_v2/openpi/examples_distributed/pi0_sort_mix.sh
...
source /x2robot_v2/xinyuanfang/projects_v2/openpi/.venv/bin/activate
...
cd $(pwd)
XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 uv run scripts/train_x2robot.py --config-name sort_mix
